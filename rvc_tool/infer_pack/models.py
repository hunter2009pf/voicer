...
from rvc_tool.infer_pack import modules
from rvc_tool.infer_pack import attentions
from rvc_tool.infer_pack import commons
...
class TextEncoder256Sim(nn.Module):
    def __init__(self, out_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout,
                 f0=True):
        super().__init__()
        self.out_channels = out_channels
        self.hidden_channels = hidden_channels
...
class SineGen(torch.nn.Module):
    ...
    def forward(self, f0, upp):
        ...
        with torch.no_grad():
            ...
            f0_buf[:, :, 0] = f0[:, :, 0]
            for idx in np.arange(self.harmonic_num): f0_buf[:, :, idx + 1] = f0_buf[:, :, 0] * (
                    idx + 2)  # idx + 2: the (idx+1)-th overtone, (idx+2)-th harmonic
            ...
            tmp_over_one *= upp
            tmp_over_one = F.interpolate(tmp_over_one.transpose(2, 1), scale_factor=upp, mode='linear',
                                         align_corners=True).transpose(2, 1)
            rad_values = F.interpolate(rad_values.transpose(2, 1), scale_factor=upp, mode='nearest').transpose(2,
                                                                                                               1)  #######
            tmp_over_one %= 1
